...
// CATS_INTEGRATION_ID=omp
// CATS_INTEGRATION_VERSION=2
// @ts-nocheck

import { createConnection } from "node:net";

const CATS_ENV = process.env.CATS_ENV;
const socketPath = process.env.CATS_SOCKET_PATH;
const paneId = process.env.CATS_PANE_ID;
const source = "cats:omp";

function enabled() {
  return CATS_ENV === "1" && !!socketPath && !!paneId;
}
...
const idleDebounceMs = parseDurationEnv("CATS_OMP_IDLE_DEBOUNCE_MS", 250);
const retryGraceMs = parseDurationEnv("CATS_OMP_RETRY_GRACE_MS", 2500);
...
export default function (pi) {
  ...
  pi.events.on("cats:blocked", (data) => {
    if (!data?.active) {
