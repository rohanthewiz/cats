...
// The hook-report ingestion seam (the port of cats's pane.report_agent* API,
// src/app/api/panes.rs): the shell hooks that `catctl integration install`
// plants in coding agents dial the unix socket in CATS_SOCKET_PATH and send
// one newline-terminated JSON request — an agent state transition, a resumable
// session id, or a release. The wire shapes and reply format are cats's, byte
...
// gives every pane CATS_ENV/CATS_PANE_ID/CATS_SOCKET_PATH via
// integration.PaneEnv, which is what arms the hooks in the first place.
//
// Arbitration (cats's src/terminal/state.rs, simplified to what the Go side
...
const defaultHookSocket = "/tmp/cats-hooks.sock"

// hookReadTimeout bounds reading the single request line (cats's
...
// Hook API method names (cats src/api/schema.rs).
const (
...
// hookError is a reply error; nil means ok. Codes are cats's snake_case set.
...
func serveHooks(o *orch, socket string) (cleanup func(), err error) {
	...
	log.Printf("catway: hook-report API listening on %s", socket)
	return func() { _ = l.Close(); _ = os.Remove(socket) }, nil
}

// serveHookConn handles one hook connection: one newline-framed request in, one
// reply out (cats's one-shot transport). A request without a trailing newline
...
// state (cats's HookAuthority). While present it outranks the daemon's
...
// (cats's AgentSessionRef): claude's SessionStart session_id and friends. Kind
...
// Reserved native state sources (cats src/agent_resume.rs): these agents'
...
var reservedNativeSources = map[string]bool{
	"cats:claude": true, "cats:codex": true, "cats:copilot": true,
	"cats:droid": true, "cats:qodercli": true, "cats:cursor": true,
}

// Full-lifecycle hook sources (cats src/detect/mod.rs): agents whose hooks
// report every transition, so a detected visible blocker never overrides them.
var fullLifecycleSources = map[string]bool{
	"cats:pi": true, "cats:omp": true, "cats:hermes": true,
	"cats:opencode": true, "cats:kilo": true,
}

// Official agent sources (cats src/agent_resume.rs is_official_agent_source):
...
var officialAgentSources = map[string]string{
	"cats:claude": "claude", "cats:codex": "codex", "cats:copilot": "copilot",
	"cats:droid": "droid", "cats:kimi": "kimi", "cats:pi": "pi",
	"cats:hermes": "hermes", "cats:opencode": "opencode",
	"cats:qodercli": "qodercli", "cats:kilo": "kilo", "cats:cursor": "cursor",
}

func sourcePairIs(m map[string]bool, source, agent string) bool {
	return m[source] && source == "cats:"+agent
}
...
func (o *orch) applyHookReport(method string, p hookReportParams) *hookError {
	...
	case methodReleaseAgent:
		// Reserved native agents are never hook-released (cats no-ops these).
		if sourcePairIs(reservedNativeSources, source, agent) {
			return nil
		}
...
// setSessionRef records a resumable session identity (cats's
...
// already held (cats's conflicting_current_session_ref — a sub-agent's or
...
// sessionRefFromReport validates a report's session fields into a ref (cats's
...
// acceptHookSeq is cats's accept_hook_report: seq is per-source monotonic; a
...
// authority and the daemon's detection (cats's recompute_effective_state):
...
// chars (cats's normalize_custom_status).
...
// CATS_PANE_ID / CATS_ENV) as the CreatePane.Env map, from
// integration.PaneEnv — the same values `catctl integration` documents to
...
// its process exits (cats: a late in-flight hook packet must not resurrect a
