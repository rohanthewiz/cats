...
func TestHookAuthorityLifecycle(t *testing.T) {
	...
	if herr := o.applyHookReport(methodReportAgent, hookReportParams{
		PaneID: pub, Source: "cats:hermes", Agent: "hermes", State: "working", Seq: seq(10),
	}); herr != nil {
	...
	if herr := o.applyHookReport(methodReportAgent, hookReportParams{
		PaneID: pub, Source: "cats:hermes", Agent: "hermes", State: "idle", Seq: seq(10),
	}); herr != nil {
	...
	if herr := o.applyHookReport(methodReleaseAgent, hookReportParams{
		PaneID: pub, Source: "cats:hermes", Agent: "hermes", Seq: seq(20),
	}); herr != nil {
	...
	if herr := o.applyHookReport(methodReportAgent, hookReportParams{
		PaneID: pub, Source: "cats:hermes", Agent: "hermes", State: "working", Seq: seq(30),
	}); herr != nil {
	...
	if herr := o.applyHookReport(methodReportAgent, hookReportParams{
		PaneID: pub, Source: "cats:hermes", Agent: "hermes", State: "working", Seq: seq(40),
...
func TestHookReservedNativeSessionOnly(t *testing.T) {
	...
	if herr := o.applyHookReport(methodReportAgentSession, hookReportParams{
		PaneID: pub, Source: "cats:claude", Agent: "claude", Seq: seq(1),
	...
	// and a *different* id for the held conversation is ignored (cats's
	...
	if herr := o.applyHookReport(methodReportAgent, hookReportParams{
		PaneID: pub, Source: "cats:claude", Agent: "claude", State: "working", Seq: seq(2),
	...
	if herr := o.applyHookReport(methodReleaseAgent, hookReportParams{
		PaneID: pub, Source: "cats:claude", Agent: "claude", Seq: seq(3),
	}); herr != nil {
		t.Fatalf("release: %+v", herr)
	}
...
func TestHookVisibleBlockerOverride(t *testing.T) {
	...
	if herr := o.applyHookReport(methodReportAgent, hookReportParams{
		PaneID: pub, Source: "cats:kimi", Agent: "kimi", State: "working", Seq: seq(1),
	}); herr != nil {
		t.Fatalf("report_agent: %+v", herr)
	}
	...
	rt.hookSeqs = nil
	if herr := o.applyHookReport(methodReportAgent, hookReportParams{
		PaneID: pub, Source: "cats:hermes", Agent: "hermes", State: "working", Seq: seq(1),
	}); herr != nil {
		t.Fatalf("report_agent: %+v", herr)
	}
...
func TestHookConflictingDetectionDropsAuthority(t *testing.T) {
	o, rt, pub := hookOrch(t)
	if herr := o.applyHookReport(methodReportAgent, hookReportParams{
		PaneID: pub, Source: "cats:opencode", Agent: "opencode", State: "working", Seq: seq(1),
	}); herr != nil {
		t.Fatalf("report_agent: %+v", herr)
	}
...
func TestHookClearedOnExit(t *testing.T) {
	o, rt, pub := hookOrch(t)
	if herr := o.applyHookReport(methodReportAgent, hookReportParams{
		PaneID: pub, Source: "cats:kilo", Agent: "kilo", State: "working", Seq: seq(1),
	}); herr != nil {
		t.Fatalf("report_agent: %+v", herr)
	}
	...
	if herr := o.applyHookReport(methodReportAgent, hookReportParams{
		PaneID: pub, Source: "cats:kilo", Agent: "kilo", State: "working", Seq: seq(2),
	}); herr != nil {
...
func TestHookReportErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{"unknown pane", methodReportAgent,
			hookReportParams{PaneID: "w9:pz", Source: "cats:hermes", Agent: "hermes", State: "working"}, "pane_not_found"},
		{"empty agent", methodReportAgent,
			hookReportParams{PaneID: pub, Source: "cats:hermes", Agent: "  ", State: "working"}, "invalid_agent"},
		{"bad state", methodReportAgent,
			hookReportParams{PaneID: pub, Source: "cats:x", Agent: "x", State: "sleeping"}, "invalid_request"},
...
func TestSessionRefFromReport(t *testing.T) {
	if ref := sessionRefFromReport("cats:claude", "claude", hookReportParams{AgentSessionID: "s1"}); ref == nil || ref.kind != "id" || ref.value != "s1" {
	...
	if ref := sessionRefFromReport("cats:pi", "pi", hookReportParams{AgentSessionPath: "/abs/path", AgentSessionID: "s1"}); ref == nil || ref.kind != "path" || ref.value != "/abs/path" {
		t.Fatalf("pi path ref: %+v", ref)
	}
	if ref := sessionRefFromReport("cats:pi", "pi", hookReportParams{AgentSessionPath: "rel/path", AgentSessionID: "s1"}); ref == nil || ref.kind != "id" {
		t.Fatalf("relative pi path must fall back to id: %+v", ref)
	}
	if ref := sessionRefFromReport("cats:codex", "codex", hookReportParams{AgentSessionPath: "/abs"}); ref != nil {
		t.Fatalf("non-pi path must be rejected: %+v", ref)
	}
	if ref := sessionRefFromReport("cats:claude", "claude", hookReportParams{AgentSessionID: "bad\x01id"}); ref != nil {
...
func TestPaneEnvMap(t *testing.T) {
	...
	if m["CATS_SOCKET_PATH"] != "/tmp/h.sock" || m["CATS_PANE_ID"] != "w1:p3" || m["CATS_ENV"] != "1" {
		t.Fatalf("env map: %v", m)
	}
	if m = paneEnvMap("/tmp/h.sock", 7, ""); m["CATS_PANE_ID"] != "p_7" {
...
// End-to-end over the socket: cats's exact framing and reply shapes.
func TestServeHooksEndToEnd(t *testing.T) {
	...
	m := send(`{"id":"cats:claude:123:abc","method":"pane.report_agent_session","params":{"pane_id":"` + pub + `","source":"cats:claude","agent":"claude","seq":42,"agent_session_id":"sess-1"}}`)
	if string(m["id"]) != `"cats:claude:123:abc"` || string(m["result"]) != `{"type":"ok"}` {
		t.Fatalf("ok reply: %v", m)
	}

	// Unknown pane → cats's error envelope.
	m = send(`{"id":"x","method":"pane.report_agent","params":{"pane_id":"w9:pz","source":"cats:hermes","agent":"hermes","state":"working"}}`)
