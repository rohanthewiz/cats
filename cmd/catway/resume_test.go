...
// The per-agent plan table, byte for byte (cats agent_resume::plan): note
...
func TestResumeArgvTable(t *testing.T) {
	cases := []struct {
		...
		{"cats:claude", "claude", "id", "s1", []string{"claude", "--resume", "s1"}},
		{"cats:codex", "codex", "id", "s2", []string{"codex", "resume", "s2"}},
		{"cats:copilot", "copilot", "id", "s3", []string{"copilot", "--resume=s3"}},
		{"cats:droid", "droid", "id", "s4", []string{"droid", "--resume", "s4"}},
		{"cats:kimi", "kimi", "id", "s5", []string{"kimi", "--session", "s5"}},
		{"cats:pi", "pi", "id", "s6", []string{"pi", "--session", "s6"}},
		{"cats:pi", "pi", "path", "/tmp/pi.sess", []string{"pi", "--session", "/tmp/pi.sess"}},
		{"cats:hermes", "hermes", "id", "s7", []string{"hermes", "--resume", "s7"}},
		{"cats:opencode", "opencode", "id", "s8", []string{"opencode", "--session", "s8"}},
		{"cats:qodercli", "qodercli", "id", "s9", []string{"qodercli", "--resume", "s9"}},
		{"cats:kilo", "kilo", "id", "s10", []string{"kilo", "--session", "s10"}},
		{"cats:cursor", "cursor", "id", "s11", []string{"cursor-agent", "--resume", "s11"}},
	}
	for _, c := range cases {
...
func TestResumeArgvRejects(t *testing.T) {
	...
	cases := []struct {
		...
		{"source/agent mismatch", "cats:claude", "codex", "id", "s1"},
		{"path for non-pi", "cats:claude", "claude", "path", "/tmp/x"},
		{"relative pi path", "cats:pi", "pi", "path", "tmp/x"},
		{"empty value", "cats:claude", "claude", "id", ""},
		{"oversized id", "cats:claude", "claude", "id", string(long)},
		{"control chars", "cats:claude", "claude", "id", "s\x001"},
		{"unknown kind", "cats:claude", "claude", "ref", "s1"},
	}
	for _, c := range cases {
...
func TestPlanResume(t *testing.T) {
	saved := map[uint32]persist.AgentSession{
		3: idRef("cats:claude", "claude", "shared"),
		1: idRef("cats:claude", "claude", "shared"), // same conversation — lower id wins
		5: idRef("cats:hermes", "hermes", "other"),
...
func TestCreatePaneConsumesResumePlan(t *testing.T) {
	...
	o.restoredAgents[pid] = idRef("cats:claude", "claude", "sess-live")
	rt := o.panes[pid]
	rt.created = false
	...
	if rt.agentSession == nil || rt.agentSession.value != "sess-live" || rt.agentSession.source != "cats:claude" {
...
func TestSaveNowPersistsAgentSessions(t *testing.T) {
	...
	want := idRef("cats:claude", "claude", "sess-live")
...
func TestExitClearsAgentSession(t *testing.T) {
	o, rt, _ := hookOrch(t)
	rt.agentSession = &agentSessionRef{source: "cats:claude", agent: "claude", kind: "id", value: "sess"}
	o.restoredAgents[rt.id] = idRef("cats:claude", "claude", "sess")
...
// Detection contradicting the held ref clears it (cats's set_detected_state
...
func TestReconcileAdoptionDropsPlanKeepsRef(t *testing.T) {
	...
	o.post(func() {
		...
		o.restoredAgents[pid] = idRef("cats:claude", "claude", "sess")
		close(done)
	})
	<-done
