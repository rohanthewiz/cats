...
// Agent-session resume on cold restore — the port of cats's src/agent_resume.rs
...
//
// One deliberate adaptation: cats spawns a shell, waits up to 750 ms for the
// host terminal theme, then types the shell-quoted resume command into it. The
// catway instead execs the plan argv directly via create_pane.command/args —
// the seam cathost already honors — so the id stays argv data (never shell
...
// resumeArgv is cats's agent_resume::plan: the exact per-agent resume command
...
// resumeDedupeKey identifies one agent conversation across panes (cats's
...
// enabled, builds each pane's resume argv (cats's pane_restore_startup):
//
...
//     (cats: a duplicate pane gets no plan and no rehydrated ref).
//   - plans: pane → resume argv, first pane wins a shared conversation. Cats
//     dedupes in restore traversal order; the catway uses ascending pane id —
