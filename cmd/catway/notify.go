...
// front-end's job: unlike cats, which knows its host terminal's focus, the
// catway serves many browsers and each knows its own focus/visibility — so
// the server always sends and each client decides (cats's rule: suppress when
...
func (o *orch) onPaneAgent(ev orchestration.PaneAgent) {
	...
	// (cats's set_detected_state rules): a different agent is on screen, or
	...
	// dropped (cats: the detector is looking at the live screen; the hook
	// claim is stale).
...
func (o *orch) publishAgent(rt *paneRuntime) {
	...
	// the viewport. cats also suppresses on-viewport completions only while
...
// notifyContext locates a pane and renders cats's notification context:
