...
// Command herdrapp is the native desktop launcher for herdr: a thin Go
...
// in app.json:
//
//   - local  — supervise the in-bundle daemons (termhost -persistent + gateway
//     --auth none on loopback) and show their UI in the window. Fully offline;
//     this is the "self-contained" Herdr.app (make macapp).
//   - remote — a thin client: start no daemons, point the window at a remote
//     gateway URL (a relay host or a direct LAN/VPN address). The gateway's own
//     login page collects the password and the webview persists the session
//     cookie across launches. This is Herdr Client.app (make macapp-client).
//
...
// bundled gateway/termhost binaries. It is macOS-only (WebKit + a native menu
...
// Herdr.app, "remote" for the thin client. app.json overrides it at runtime.
...
// cgo-exported herdrappCleanup), or a SIGINT/SIGTERM. sync.Once is the single
...
func runLocal(_ appConfig) {
	b, err := startBackend()
	if err != nil {
		showError("Could not start herdr", err.Error())
		return
	}
...
// a remote gateway. On first run (no saved URL) it shows a small connect form;
// the bound herdrConnect callback persists the entered URL and navigates the
...
func runRemote(cfg appConfig) {
	...
	w := newWindow(windowTitle)
	defer w.Destroy()
	if err := w.Bind("herdrConnect", func(rawURL string) {
