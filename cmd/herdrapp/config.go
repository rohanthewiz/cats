...
// from the gateway's ~/.config/herdr/config.yaml. It records which mode the
...
type appConfig struct {
	...
	// to a gateway URL). An empty value falls back to the build-time defaultMode.
	Mode   string       `json:"mode"`
	Remote remoteTarget `json:"remote"`
}

// remoteTarget is the gateway a thin client connects to: a relay host
...
// (app.json): ~/Library/Application Support/herdr, the conventional home for a
...
func appDataDir() (string, error) {
	...
	return filepath.Join(home, "Library", "Application Support", "herdr"), nil
}
