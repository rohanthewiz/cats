...
// from herdr's cli/integration.rs: same subcommands, usage strings, exit
...
// runIntegration dispatches `herdrctl integration <subcommand> ...` and
...
func integrationStatus(args []string) int {
	...
	case len(args) == 1 && args[0] == "--outdated-only":
		outdatedOnly = true
	default:
		fmt.Fprintln(os.Stderr, "usage: herdrctl integration status [--outdated-only]")
		return 2
	}
...
func parseIntegrationTarget(args []string, action string) (integration.Target, bool) {
	if len(args) != 1 {
		fmt.Fprintf(os.Stderr, "usage: herdrctl integration %s <%s>\n", action, integrationTargetsUsage)
		return 0, false
	}
...
func printIntegrationHelp() {
	fmt.Fprintln(os.Stderr, "herdrctl integration commands:")
	for _, action := range []string{"install", "uninstall"} {
		for _, target := range integration.AllTargets() {
			fmt.Fprintf(os.Stderr, "  herdrctl integration %s %s\n", action, target.Label())
		}
	}
	fmt.Fprintln(os.Stderr, "  herdrctl integration status [--outdated-only]")
}
