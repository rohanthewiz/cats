...
// --accent from cmd/catway/web/css/01-theme.css and the tile is that theme's two
...
`, int(fillFrac*100), tileTop, tileBottom, tx, ty, scale, logoPath, themeAccent)
	...
	// The sidebar mark in cmd/catway/web/mark.svg is this path. It is a separate
	// simplification of the same trace rather than a reuse of the icon's,
	// because the icon needs full fidelity and the glyph needs to be small
	// enough to ship inside a page that is served whole.
	//
  ...
  <!-- The sidebar glyph: mark 06 traced from the reference and simplified for
       ...
       scripts/icon/gen-trace; if it is regenerated, the copy in
       cmd/catway/web/mark.svg must be replaced too.
