...
| `runbook_runs` | every runbook run in flight — `name`, `source` (`control` or `trigger`), the `trigger` event name where there is one, and `started_at`. Pushed when a run takes its concurrency slot and again when it releases it, plus once on client init, including when the set is empty — a client that reconnects across the end of a run has a mark to turn **off**. The whole set rather than start/stop deltas, so a dropped or coalesced message converges on the next one instead of leaving a row lit for a run that ended. A broadcast for the reason `record` is one, and a browser message rather than a control-API event for a second: events feed the runbook triggers, so an event per run start would hand a runbook an event that starting a runbook produces. `runbook_finished` is the event-side report, once per run, at the end |
